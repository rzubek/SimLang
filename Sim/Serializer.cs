...
{
    /// <summary>
    /// Wrapper class for intermediate serialized objects. This is the result of serialization
    /// before it's passed over to printing as text, or the result of parsing text
    /// before it's passed over to deserialization and turned into a strongly-typed class instance.
    ///
    /// Serialized objects are always simple values, which means either (1) one a few specific primitives:
    /// bool, long/ulong, string, double, (2) an ArrayList of simple values, or
    /// (3) a Hashmap mapping from simple values to simple values.
    /// 
    /// </summary>
    ...
    /// <summary>
    /// Class for exceptions thrown during the serialization / deserialization phase.
    /// </summary>
    ...
    /// <summary>
    /// Stores a variety of configuration options for the serializer.
    /// </summary>
    ...
    /// <summary>
    /// Serializes strongly-typed instances into serialized objects and back out into class instances.
    ...
    /// - Each serializer instance is only safe for use from a single thread - before
    ///   using from another thread, make a clone.
    ///
    /// Some details:
    ...
    public class Serializer
    {
        ...
        {
            public List<NamespaceDef> ImplicitNamespaces = new List<NamespaceDef>();
        ...
        /// <summary>
        /// Creates a duplicate copy of this serializer instance, for use on another thread.
        /// </summary>
        ...
        /// <summary>
        /// Serializes a given strongly-typed object into an intermediate untyped object.
        /// </summary>
        public object Serialize (object source, bool specifyValueTypes = false) {
        ...
        private object TrySerializeUntypedCollection (object source, Type sourceType) {
            ...
            return source switch {
                IDictionary idict => SerializeDictionary(idict, shouldEmitTypes),
                IEnumerable ien => SerializeEnumerable(ien, shouldEmitTypes),
                _ => null,
            };
        }
        ...
        private Dictionary<Type, Func<object, object>> PrimitiveConverters = new Dictionary<Type, Func<object, object>>() {
            { typeof(bool), obj => Convert.ToBoolean(obj) },
            { typeof(ulong), obj => Convert.ToUInt64(obj) },
            { typeof(long), obj => Convert.ToInt64(obj) },
            { typeof(uint), obj => Convert.ToUInt32(obj) },
            { typeof(int), obj => Convert.ToInt32(obj) },
            { typeof(ushort), obj => Convert.ToUInt16(obj) },
            { typeof(short), obj => Convert.ToInt16(obj) },
            { typeof(byte), obj => Convert.ToByte(obj) },
            { typeof(sbyte), obj => Convert.ToSByte(obj) },
            { typeof(double), obj => Convert.ToDouble(obj) },
            { typeof(float), obj => Convert.ToSingle(obj) },
            { typeof(char), obj => Convert.ToChar(obj) },
            { typeof(string), obj => Convert.ToString(obj) },
            { typeof(DateTime), obj => DateTime.FromBinary(Convert.ToInt64(obj)) },
        };
        ...
        private object TryParseAndCacheEnum (string str, Type enumType) {
            ...
            // see if it's cached
            var result = FindOrNull(_TypeAndNameToEnum, enumType, str);
            ...
            Add(_TypeAndNameToEnum, enumType, str, result);
            return result;


            static object FindOrNull (Dictionary<Type, Dictionary<string, object>> typeAndNameToEnum, Type enumType, string name) {
                bool gotNames = typeAndNameToEnum.TryGetValue(enumType, out Dictionary<string, object> nameToEnum);
                if (!gotNames) { return null; }

                bool gotEnum = nameToEnum.TryGetValue(name, out object result);
                if (!gotEnum) { return null; }

                return result;
            }

            static void Add (Dictionary<Type, Dictionary<string, object>> typeAndNameToEnum, Type enumType, string name, object result) {
                bool gotNames = typeAndNameToEnum.TryGetValue(enumType, out Dictionary<string, object> nameToEnum);
                if (!gotNames) {
                    nameToEnum = typeAndNameToEnum[enumType] = new Dictionary<string, object>();
                }

                nameToEnum.Add(name, result);
            }
        }
        ...
        public void AddImplicitNamespace (string prefix, bool isNamespace) =>
            _Settings.ImplicitNamespaces.Add(new NamespaceDef() { prefix = prefix, isclass = !isNamespace });

        public void RemoveImplicitNamespace (string prefix) =>
            _Settings.ImplicitNamespaces.RemoveAll((NamespaceDef def) => def.prefix == prefix);
        ...
        /// <summary>
        /// Creates a duplicate copy of the given object, by serializing and deserializing it.
        /// </summary>
        ...
        private static class TypeUtils
        {
            ...
            public static Type GetSerializableTypeOrNull (MemberInfo i) {
                if (i is PropertyInfo) {
                    ...
                    return (pi.CanRead && pi.CanWrite && pi.SetMethod.IsPublic && pi.GetMethod.IsPublic) ? pi.PropertyType : null;
