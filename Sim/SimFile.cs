...
namespace SomaSim.SimLang
{
    ...
    public class SimException : Exception
    {
        ...
        internal SimException (Exception e, SimFile.ParseContext c, string message) : base(message, e) { SetContext(c); }
        internal SimException (SimFile.ParseContext c, string message) : base(message) { SetContext(c); }

        internal SimException (Exception e, SimFile.PrintContext c, string message) : base(message, e) { SetContext(c); }
        internal SimException (SimFile.PrintContext c, string message) : base(message) { SetContext(c); }

        private void SetContext (SimFile.ParseContext c) {
            char[] buffer = new char[128];
            int read = c.reader.ReadBlock(buffer, 0, buffer.Length);
            _context = $"Parse Context: {((read > 0) ? new string(buffer) : "(???)")}";
        }

        private void SetContext (SimFile.PrintContext c) {
    ...
    /// <summary>
    /// Performs writing out serialized values into a text file, and reading those text files
    /// back into a serialized value that can be deserialized.
    /// </summary>
    public class SimFile
    {
        /// <summary>
        /// Global settings for parsing text into tokens and tokens into values.
        /// </summary>
        ...
        /// <summary>
        /// Global settings for printing values out to text.
        /// </summary>
        ...
        #region Top-level API, if you don't need any specialized forms, use this

        public static string Serialize<T> (T value, Serializer serializer = default) {
            var s = serializer ?? new Serializer();
            var intermediate = s.Serialize(value);
            return Print(intermediate);
        }

        public static T Deserialize<T> (string simfile, Serializer serializer = default) {
            var s = serializer ?? new Serializer();
            var intermediate = Parse(simfile);
            return s.Deserialize<T>(intermediate);
        }
        ...
        private static object ParseValue (ParseContext c) {
            ...
            if (ch == TOKEN_BOOL_OR_NULL_START) {
                ...
                throw new SimException(c, string.Format("Unexpected character {0} / {1:x8}", ch, next));
            }
        }
        ...
        private static object ParseBoolOrNull (ParseContext c) {
            ...
            return token switch {
                ...
                "false" => false,
                "null" => null,
                _ => magic + token,// just return it as is
            };
        }
        ...
        private static object ParseNumber (ParseContext c) {
            ...
            try {
                ...
                throw new SimException(e, c, "Unknown number format in " + token);
            } catch (Exception e) {
                throw new SimException(e, c, "Failed to parse as number " + token);
            }
        }
        ...
        private static object ParseQuotedString (ParseContext c) {
            ...
            do {
                if (c.reader.Peek() == -1) {
                    throw new SimException(c, "String not terminated with value " + c.GetStringAndResetBuilder());
                }
        ...
        private static object ParseMacro (ParseContext c) {
            ...
            do {
                if (c.reader.Peek() == -1) {
                    throw new SimException(c, "Macro not terminated with value " + c.GetStringAndResetBuilder());
                }
        ...
        private static object ParseDictionary (ParseContext c) {
            ...
            if (pairs.Count % 2 == 1) {
                ...
                throw new SimException(c, "Dictionary needs an even number of elements (keys and values) for dictionary starting with " + hint);
            }
            ...
            for (int i = 0; i < pairs.Count; i += 2) {
                ...
                if (results.ContainsKey(key)) {
                    throw new SimException(c, "Object definition contains duplicate copies of key: " + key);
                }
                results[key] = val;
            }
        ...
        private static ArrayList ParseAsListUntil (ParseContext c, char terminator, ArrayList results) {
            while (true) {
                ...
                if (c.reader.Peek() == -1) {
                    ...
                    throw new SimException(c, "Collection not terminated correctly, starting with " + hint);
                }
        ...
        internal class PrintContext
        {
            ...
            public readonly bool alphasort;
            public int indent;

            public PrintContext (bool compressed, bool alphasort) {
                this.sb = new StringBuilder();
        ...
        public static string Print (object data) => Print(data, false, true);

        public static string Print (object data, bool compressed, bool alphasort) {
            var c = new PrintContext(compressed, alphasort);
            Print(data, c);
        ...
        private static void Print (object data, PrintContext c) {
            ...
            if (!done) {
                throw new SimException(c, "Don't know how to print object: " + data);
            }
        }
        ...
        private static string FormatFloatingPoint (object data) {
            // since sim doesn't support scientific notation, we want to make sure not to print it
            ...
            if (maxdecimals >= 0) {
                ...
                if (dotpos >= 0) {
                    ...
                    if (decimals > maxdecimals) {
                        ...
                        candidate = candidate[..newlength].TrimEnd('0').TrimEnd('.');
                    }
                }
            }
        ...
        private static bool PrintList (object data, PrintContext c) {
            ...
            bool multiline = ContainsComplexHashtable(list);

            c.sb.Append(TOKEN_LIST_START);
            c.sb.Append(" ");
            c.IndentMore();

            AddNewLineOrSpaceIf(multiline, c, true);

            for (int i = 0, end = list.Count - 1; i <= end; ++i) {
                // maybe insert an element separator
                AddNewLineOrSpaceIf(i > 0, c, multiline);
                Print(list[i], c);
            }

            c.IndentLess();
            AddNewLineOrSpace(c, multiline);
        ...
        private static bool PrintDictionary (object data, PrintContext c) {
            ...
            bool multiline = IsComplexHashtable(dict);

            var keys = dict.Keys;

            if (c.alphasort) { var copy = new ArrayList(keys); copy.Sort(); keys = copy; }
            ...
            foreach (var key in keys) {
                var value = dict[key];
                var last = ++i == dict.Count;

                Print(key, c);
                AddNewLineOrSpace(c, false);

                Print(value, c);
                AddNewLineOrSpaceIf(!last, c, multiline);
            }
        ...
        private static void AddNewLineOrSpaceIf (bool test, PrintContext c, bool line) {
            if (test) {
                AddNewLineOrSpace(c, line);
            }
        }

        private static void AddNewLineOrSpace (PrintContext c, bool line) {
            if (line) {
